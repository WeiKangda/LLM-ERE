...
class Document:
    def __init__(self, data, ignore_nonetype=False):
        ...
        self.subevent_distance = []
        self.events_number = len(self.events)
        self.temporal_labels = self.get_relation_labels(self.temporal_relations, TEMPREL2ID, 'temporal', ignore_timex=False)
        self.causal_labels = self.get_relation_labels(self.causal_relations, CAUSALREL2ID, 'causal', ignore_timex=True)
        self.subevent_labels = self.get_relation_labels(self.subevent_relations, SUBEVENTREL2ID, 'subevent', ignore_timex=True)
    ...
    def get_coref_labels(self, data):
        ...
        if "events" in data:
            for event in data["events"]:
                ...
                clusters.append([m for m in event["mention"]])
        else:
        ...
        for cluster in clusters:
                ...
                for event_1 in cluster:
                    for event_2 in cluster:
                        self.coref_distance.append(np.absolute(event_1["sent_id"] - event_2["sent_id"]))

        return label_group
 
    def get_relation_labels(self, relations, REL2ID, rtype, ignore_timex=True):
        pair2rel = {}
        ...
        for i, e1 in enumerate(self.events):
            for j, e2 in enumerate(self.events):
                ...
                if pair2rel.get((e1["id"], e2["id"]), REL2ID["NONE"]) != REL2ID["NONE"]:
                    if rtype == 'temporal':
                        self.temporal_distance.append(np.absolute(e1["sent_id"] - e2["sent_id"]))
                    if rtype == 'causal':
                        self.causal_distance.append(np.absolute(e1["sent_id"] - e2["sent_id"]))
                    if rtype == 'subevent':
                        self.subevent_distance.append(np.absolute(e1["sent_id"] - e2["sent_id"]))
        #if rtype == 'temporal':
        #    print(f"Doc {self.id} temporal average distance: {np.mean(self.temporal_distance)}")
        #    print(f"Doc {self.id} temporal distance std: {np.std(self.temporal_distance)}")
        #if rtype == 'causal':
        #    print(f"Doc {self.id} causal average distance: {np.mean(self.causal_distance)}")
        #    print(f"Doc {self.id} causal distance std: {np.std(self.causal_distance)}")
        #if rtype == 'subevent':
        #    print(f"Doc {self.id} subevent average distance: {np.mean(self.subevent_distance)}")
        #    print(f"Doc {self.id} subevent distance std: {np.std(self.subevent_distance)}")
...
class myDataset(Dataset):
    ...
    def load_examples(self, data_dir, split):
        self.examples = []
        self.corefernce_distances = []
        self.temporal_distances = []
        self.causal_distances = []
        self.subevent_distances = []
        ...
        for line in lines:
            ...
            self.corefernce_distances.extend(doc.coref_distance)
            self.temporal_distances.extend(doc.temporal_distance)
            self.causal_distances.extend(doc.causal_distance)
            self.subevent_distances.extend(doc.subevent_distance)
            self.total_events_number += doc.events_number
        
        print(f"Total #Mention: {self.total_events_number}")
        print(f"Corefernce number: {len(self.corefernce_distances)}")
        print(f"Overall corefernce average distance: {np.nanmean(self.corefernce_distances)}")
        print(f"Overall corefernce median distance: {np.nanmedian(self.corefernce_distances)}")
        print(f"Overall corefernce distance std: {np.nanstd(self.corefernce_distances)}")
        print(f"Temporal number: {len(self.temporal_distances)}")
        print(f"Overall temporal average distance: {np.nanmean(self.temporal_distances)}")
        print(f"Overall temporal median distance: {np.nanmedian(self.temporal_distances)}")
        print(f"Overall temporal distance std: {np.nanstd(self.temporal_distances)}")
        print(f"Causal number: {len(self.causal_distances)}")
        print(f"Overall causal average distance: {np.nanmean(self.causal_distances)}")
        print(f"Overall causal median distance: {np.nanmedian(self.causal_distances)}")
        print(f"Overall causal distance std: {np.nanstd(self.causal_distances)}")
        print(f"Subevent number: {len(self.subevent_distances)}")
        print(f"Overall subevent average distance: {np.nanmean(self.subevent_distances)}")
        print(f"Overall subevent median distance: {np.nanmedian(self.subevent_distances)}")
        print(f"Overall subevent distance std: {np.nanstd(self.subevent_distances)}")

        sns.set_style('whitegrid')
        sns.distplot(self.corefernce_distances)
        plt.title(f"Coreference Relations' Distance for {split} set")
        plt.xlabel("Distance in # of Sentences", fontsize=16)
        plt.ylabel("Percentage", fontsize=16)
        #plt.xlim(0, 35)
        #plt.ylim(0, 0.5)
        plt.tight_layout()
        plt.savefig(f"./images/corefernce_distances_{split}.pdf")
        plt.savefig(f"./images/corefernce_distances_{split}.png")
        plt.close()

        sns.set_style('whitegrid')
        sns.distplot(self.temporal_distances)
        plt.title(f"Temporal Relations' Distance for {split} set")
        plt.xlabel("Distance in # of Sentences", fontsize=16)
        plt.ylabel("Percentage", fontsize=16)
        #plt.xlim(0, 80)
        #plt.ylim(0, 0.5)
        plt.tight_layout()
        plt.savefig(f"./images/temporal_distances_{split}.pdf")
        plt.savefig(f"./images/temporal_distances_{split}.png")
        plt.close()
        
        sns.set_style('whitegrid')
        sns.distplot(self.causal_distances)
        plt.title(f"Causal Relations' Distance for {split} set")
        plt.xlabel("Distance in # of Sentences", fontsize=16)
        plt.ylabel("Percentage", fontsize=16)
        #plt.xlim(0, 60)
        #plt.ylim(0, 0.5)
        plt.tight_layout()
        plt.savefig(f"./images/causal_distances_{split}.pdf")
        plt.savefig(f"./images/causal_distances_{split}.png")
        plt.close()

        sns.set_style('whitegrid')
        sns.distplot(self.subevent_distances)
        plt.title(f"Subevent Relations' Distance for {split} set")
        plt.xlabel("Distance in # of Sentences", fontsize=16)
        plt.ylabel("Percentage", fontsize=16)
        #plt.xlim(0, 60)
        #plt.ylim(0, 0.5)
        plt.tight_layout()
        plt.savefig(f"./images/subevent_distances_{split}.pdf")
        plt.savefig(f"./images/subevent_distances_{split}.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    dataloader = get_dataloader(tokenizer, "valid", shuffle=False, max_length=256)
