...
class Document:
    def __init__(self, data, ignore_nonetype=False):
        ...
        self.event_id_pairs = self.get_event_id_pairs()
    ...
    def get_event_id_pairs(self):
        pairs = []
        for event in self.events:
            self.mentionid2mention[event["id"]] = event
        ...
        assert len(pairs) == len(self.events) ** 2 - len(self.events)
        return pairs
...
class myDataset(Dataset):
    ...
    def tokenize(self):
        ...
        for example in tqdm(self.examples, desc="tokenizing"):
            ...
            tokenized = {"input_ids": input_ids, "attention_mask": None, "event_spans": event_spans, "coref_labels": example.coref_labels, "temporal_labels": example.temporal_labels, "causal_labels": example.causal_labels, "subevent_labels": example.subevent_labels, "events_idx": example.events_idx, "doc_id": example.id, "event_id_pairs": example.event_id_pairs, "mentionid2mention": example.mentionid2mention}
...
def collator(data):
    collate_data = {"input_ids": [], "attention_mask": [], "event_spans": [], "splits": [0], "coref_labels": [], "temporal_labels": [],"causal_labels": [], "subevent_labels": [], "events_idx": [], "doc_id": [], "event_id_pairs": [], "mentionid2mention": []}
    for d in data:
        for k in d:
