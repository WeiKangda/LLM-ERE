...
def evaluate(model, dataloader, desc=""):
    ...
    for data in tqdm(dataloader, desc=desc):
        ...
        coref_scores, temporal_scores, causal_scores, subevent_scores = model.predict(data)
        # coreference ###########################
    ...
    with torch.no_grad():
        for data in tqdm(dataloader, desc="Predict"):
            for k in data:
                if isinstance(data[k], torch.Tensor):
                    data[k] = to_cuda(data[k])
            _, _, scores, _ = model.predict(data)
            labels = data["causal_labels"]
            scores = scores.view(-1, scores.size(-1))
            labels = labels.view(-1)
            pred = torch.argmax(scores, dim=-1)
    ...
    with torch.no_grad():
        for data in tqdm(dataloader, desc="Predict"):
            for k in data:
                if isinstance(data[k], torch.Tensor):
                    data[k] = to_cuda(data[k])
            _, _, _, scores = model.predict(data)
            labels = data["subevent_labels"]
            scores = scores.view(-1, scores.size(-1))
            labels = labels.view(-1)
            pred = torch.argmax(scores, dim=-1)
    ...
    with torch.no_grad():
        for data in tqdm(dataloader, desc="Predict"):
            for k in data:
                if isinstance(data[k], torch.Tensor):
                    data[k] = to_cuda(data[k])
            probs, _, _, _ = model.predict(data)
    ...
    with torch.no_grad():
        for data in tqdm(dataloader, desc="Predict"):
            for k in data:
                if isinstance(data[k], torch.Tensor):
                    data[k] = to_cuda(data[k])
            _, scores, _, _ = model.predict(data)
            labels = data["temporal_labels"]
            scores = scores.view(-1, scores.size(-1))
            labels = labels.view(-1)
            pred = torch.argmax(scores, dim=-1)
...
if __name__ == "__main__":
    ...
    output_dir = Path(f"./output/rgcn/{args.seed}/MAVEN-ERE")
    ...
    if not args.eval_only:
        ...
        scorer_param += [p for p in model.temporal_rgcn_scorer.parameters() if p.requires_grad]
        scorer_param += [p for p in model.causal_rgcn_scorer.parameters() if p.requires_grad]
        scorer_param += [p for p in model.subevent_rgcn_scorer.parameters() if p.requires_grad]
        scorer_param += [p for p in model.coref_rgcn_scorer.parameters() if p.requires_grad]
        scorer_param += [p for p in model.rgcn.parameters() if p.requires_grad]
        ...
        for epoch in range(args.epochs):
            for data in tqdm(train_dataloader, desc=f"Training epoch {epoch}"):
                ...
                for k in data:
                    if isinstance(data[k], torch.Tensor):
                        data[k] = to_cuda(data[k])
                coref_scores, temporal_scores, causal_scores, subevent_scores, temporal_noisy_scores, causal_noisy_scores, subevent_noisy_scores = model(data)
                tmp_coref_loss=0.0
                ...
                for i in range(len(coref_scores)):
                    ...
                    tmp_coref_loss += tmp
                    pred_clusters, pred_event2cluster = get_predicted_clusters(prob)
                    #print(f"pred_clusters: {pred_clusters}")
                    #print(f"pred_event2cluster: {pred_event2cluster}")
                    gold_event2cluster = get_event2cluster(labels)
                    #print(f"gold_event2cluster: {gold_event2cluster}")
                    assert len(pred_event2cluster) == len(gold_event2cluster), print(pred_event2cluster, gold_event2cluster)
                    eval_result = EvalResult(labels, gold_event2cluster, pred_clusters, pred_event2cluster)
                    coref_train_eval_results.append(eval_result)
                ...
                scores = temporal_scores
                scores = scores.view(-1, scores.size(-1))
                noisy_scores = temporal_noisy_scores
                noisy_scores = noisy_scores.view(-1, noisy_scores.size(-1))
                labels = labels.view(-1)
                tmp = Loss(scores, labels)
                noisy_tmp = Loss(noisy_scores, labels)
                loss += args.temporal_rate * (tmp + noisy_tmp)
                temp_losses.append(tmp.item())
                temp_losses.append(noisy_tmp.item())
                pred = torch.argmax(scores, dim=-1)
                temporal_pred_list.extend(pred[labels>=0].cpu().numpy().tolist())
                temporal_label_list.extend(labels[labels>=0].cpu().numpy().tolist())
                ...
                scores = causal_scores
                scores = scores.view(-1, scores.size(-1))
                noisy_scores = causal_noisy_scores
                noisy_scores = noisy_scores.view(-1, noisy_scores.size(-1))
                labels = labels.view(-1)
                tmp = Loss(scores, labels)
                noisy_tmp = Loss(noisy_scores, labels)
                loss += args.causal_rate * (tmp + noisy_tmp)
                causal_losses.append(tmp.item())
                causal_losses.append(noisy_tmp.item())
                pred = torch.argmax(scores, dim=-1)
                causal_pred_list.extend(pred[labels>=0].cpu().numpy().tolist())
                causal_label_list.extend(labels[labels>=0].cpu().numpy().tolist())
                ...
                scores = subevent_scores
                scores = scores.view(-1, scores.size(-1))
                noisy_scores = subevent_noisy_scores
                noisy_scores = noisy_scores.view(-1, noisy_scores.size(-1))
                labels = labels.view(-1)
                tmp = Loss(scores, labels)
                noisy_tmp = Loss(noisy_scores, labels)
                loss += args.subevent_rate * (tmp + noisy_tmp)
                subevent_losses.append(tmp.item())
                subevent_losses.append(noisy_tmp.item())
                pred = torch.argmax(scores, dim=-1)
                subevent_pred_list.extend(pred[labels>=0].cpu().numpy().tolist())
                subevent_label_list.extend(labels[labels>=0].cpu().numpy().tolist())
