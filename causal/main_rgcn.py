...
from utils.gat import Model_GAT
from utils.rgcn import Model_RGCN
import torch
import random
import numpy as np
...
if __name__ == "__main__":
    ...
    output_dir = Path(f"./output/rgcn/{args.seed}/maven_ignore_none_{args.ignore_nonetype}_{args.sample_rate}")
    ...
    print("loading model...")
    model = Model_RGCN(len(tokenizer), out_dim=label_num)
    model = to_cuda(model)
    ...
    if not args.eval_only:
        ...
        #print(f"Training epoch {epoch}: {num_0}, {num_1}, {num_2}, {num_3}, {num_4}, {num_5}, {num_6}")
