import sys
sys.path.insert(0, '/scratch/user/kangda')
import os
...
def main(
    ...
    docs = []
    if test: 
        output_dir = "./output/llama2_iterative_prediction/test"
    else:
        output_dir = "./output/llama2_iterative_prediction/valid"
    ...
    for i, doc in enumerate(tqdm(docs, desc="Predicting")):
        ...
        #    exit()
        print(doc["id"])
        if doc["id"] == "d791d9d612faaaf9ca63e206aee82489" or doc["id"] == "d8ec928eebf398848c96f7870385d616" or doc["id"] == "14a079b9770a338d7da5a996d7a82a83":
            continue
            ...
            if relation == "coreference":
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    if j >= len(example_doc["sentences"]) - 1:
                        prompt_context = [example_doc["sentences"][index] for index in range(len(example_doc["sentences"]))]
                        prompt_context = ' '.join(prompt_context)
                        prompt_answer = example_doc["coref_answer_inputs"][-1]
                    else:
                        prompt_context = [example_doc["sentences"][index] for index in range(j)] #example_doc["temporal_context_inputs"][-1]
                        prompt_context = ' '.join(prompt_context)
                        prompt_answer = example_doc["coref_answer_inputs"][j]
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    if j >= len(example_doc["sentences"]) - 1:
                        prompt_context = [example_doc["sentences"][index] for index in range(len(example_doc["sentences"]))]
                        prompt_context = ' '.join(prompt_context)
                        prompt_answer = example_doc["temporal_answer_inputs"][-1]
                    else:
                        prompt_context = [example_doc["sentences"][index] for index in range(j)] #example_doc["temporal_context_inputs"][-1]
                        prompt_context = ' '.join(prompt_context)
                        prompt_answer = example_doc["temporal_answer_inputs"][j]
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    if j >= len(example_doc["sentences"]) - 1:
                        prompt_context = [example_doc["sentences"][index] for index in range(len(example_doc["sentences"]))]
                        prompt_context = ' '.join(prompt_context)
                        prompt_answer = example_doc["causal_answer_inputs"][-1]
                    else:
                        prompt_context = [example_doc["sentences"][index] for index in range(j)] #example_doc["temporal_context_inputs"][-1]
                        prompt_context = ' '.join(prompt_context)
                        prompt_answer = example_doc["causal_answer_inputs"][j]
