...
if __name__ == "__main__":
    ...
    docs = []
    if args.test: 
        output_dir = f"./llm/output/gpt4_iterative_prediction_few_shot/{args.shot}_shot/test"
    else:
        output_dir = f"./llm/output/gpt4_iterative_prediction_few_shot/{args.shot}_shot/valid"
    ...
    example_docs = []
    with open(f"./llm/example_few_shot.jsonl")as f:
        lines = f.readlines()
            ...
            if relation == "coreference":
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    if j == 0:
                        ...
                        for mention in mentions_in_sentence:
                            ...
                            mentions_appeared.append(mention)
                        accumulated_context = [' '.join(join_punctuation(coref_tokens[index])) for index in range(j)]
                        accumulated_context.append(sentence)
                    ...
                    for delay_secs in (2**x for x in range(0, 6)):
                        ...
                        # Call openai request such as text completion
                            ...
                            for i in range(args.shot):
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['coref_context_inputs'][0]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['coref_answer_inputs'][0]}"})
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['coref_context_inputs'][1]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['coref_answer_inputs'][1]}"})
                            ...
                            doc[f"{relation}_raw_output"].append(completion.choices[0].message["content"])
                            break
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    if j == 0:
                        ...
                        for mention_appeared in mentions_appeared:
                            ...
                            if mentions_appeared[mention_appeared] == num:
                                num = 0
                        accumulated_context = [' '.join(join_punctuation(temporal_tokens[index])) for index in range(j)]
                        accumulated_context.append(sentence)
                        input_context = ' '.join(join_punctuation(accumulated_context))
                    ...
                    for delay_secs in (2**x for x in range(0, 6)):
                        ...
                        # Call openai request such as text completion
                            ...
                            for i in range(args.shot):
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['temporal_context_inputs'][0]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['temporal_answer_inputs'][0]}"})
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['temporal_context_inputs'][1]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['temporal_answer_inputs'][1]}"})
                            few_shot_message.append({"role": "user", "content": f"{input_context}"})
                            completion = openai.ChatCompletion.create(
                            model="gpt-4-1106-preview",
                            messages=few_shot_message
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    if j == 0:
                        ...
                        for mention_appeared in mentions_appeared:
                            ...
                            if mentions_appeared[mention_appeared] == num:
                                num = 0
                        accumulated_context = [' '.join(join_punctuation(causal_tokens[index])) for index in range(j)]
                        accumulated_context.append(sentence)
                        input_context = ' '.join(join_punctuation(accumulated_context))
                    ...
                    for delay_secs in (2**x for x in range(0, 6)):
                        ...
                        # Call openai request such as text completion
                            ...
                            for i in range(args.shot):
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['causal_context_inputs'][0]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['causal_answer_inputs'][0]}"})
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['causal_context_inputs'][1]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['causal_answer_inputs'][1]}"})
                            few_shot_message.append({"role": "user", "content": f"{input_context}"})
                            completion = openai.ChatCompletion.create(
                            model="gpt-4-1106-preview",
                            messages=few_shot_message
                ...
                for j, sentence in enumerate(doc["sentences"]):
                    ...
                    if j == 0:
                        ...
                        print(j)
                        print(doc[f"{relation}_raw_output"])
                        raw_output = doc[f"{relation}_raw_output"][j-1]
                        raw_output = raw_output.replace(" SHIFT;", "")
                        raw_output = raw_output.split(";")
                        for pair in raw_output:
                        ...
                        for mention_appeared in mentions_appeared:
                            ...
                            if mentions_appeared[mention_appeared] == num:
                                num = 0
                        accumulated_context = [' '.join(join_punctuation(subevent_tokens[index])) for index in range(j)]
                        accumulated_context.append(sentence)
                        input_context = ' '.join(join_punctuation(accumulated_context))
                    ...
                    for delay_secs in (2**x for x in range(0, 6)):
                        ...
                        # Call openai request such as text completion
                            ...
                            for i in range(args.shot):
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['subevent_context_inputs'][0]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['subevent_answer_inputs'][0]}"})
                                few_shot_message.append({"role": "user", "content": f"{example_docs[i]['subevent_context_inputs'][1]}"})
                                few_shot_message.append({"role": "assistant", "content": f"{example_docs[i]['subevent_answer_inputs'][1]}"})
                            few_shot_message.append({"role": "user", "content": f"{input_context}"})
                            completion = openai.ChatCompletion.create(
                            model="gpt-4-1106-preview",
                            messages=few_shot_message
